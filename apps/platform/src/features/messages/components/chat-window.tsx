      ...
      >
        ...
        <div className="w-full p-4 bg-white rounded-xl border border-zinc-100 flex flex-col gap-4 shadow-sm max-w-md mx-auto lg:mx-0">
          ...
          <div className="p-3 bg-stone-50 rounded-lg">
            <span className="text-[10px] font-bold text-zinc-400 uppercase block mb-1">Description</span>
            <p className="text-xs text-zinc-600 leading-relaxed">
              Kitchen faucet leak and circuit breaker tripping in the main hall area.
            </p>
          </div>
          <div className="flex items-center gap-2 text-[11px] text-zinc-500">
            <HiOutlineLocationMarker className="w-3.5 h-3.5" />
            <span>3.2 km away · Lagos Island</span>
          </div>
        </div>

        {messages.map((msg, idx) => {
          const isMe = msg.senderId === user?.id
          const isQuotation = msg.type === 'quotation'
